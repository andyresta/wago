...
cd evolution-go
cp .env.example .env
# edit GLOBAL_API_KEY di .env
docker compose up -d --build
```

Aplikasi tersedia di `http://localhost:8080` (sesuai `SERVER_PORT` di `.env`).
...
cd evolution-go
cp .env.example .env
# pastikan PostgreSQL jalan, lalu:
go mod download
go run ./cmd/evolution-go -dev
```
...
# Database (pakai host "postgres" saat deploy via docker compose)
